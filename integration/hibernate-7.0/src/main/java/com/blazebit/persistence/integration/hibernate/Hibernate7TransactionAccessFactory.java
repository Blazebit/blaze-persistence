...
public class Hibernate7TransactionAccessFactory implements TransactionAccessFactory {
    @Override
    public TransactionAccess createTransactionAccess(EntityManager entityManager) {
        try {
            return new Hibernate7EntityTransactionSynchronizationStrategy( entityManager.getTransaction(), entityManager);
        } catch (IllegalStateException ex) {
            JtaPlatform jtaPlatform = getHibernateJtaPlatform(entityManager);
            return new Hibernate7JtaPlatformTransactionSynchronizationStrategy( jtaPlatform);
        }
    }
