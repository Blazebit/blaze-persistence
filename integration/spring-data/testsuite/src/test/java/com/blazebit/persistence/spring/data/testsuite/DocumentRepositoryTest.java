...
public class DocumentRepositoryTest extends AbstractSpringTest {
    ...
    public void testFindByNameInKeysetPaginated() {
        ...
        // When
        KeysetAwarePage<DocumentAccessor> actual = DocumentAccessors.of(documentRepository.findByNameIn(new KeysetPageRequest(null, Sort.asc("id"), 0, 1), d2.getName(), d3.getName()));
        List<Long> actualIds = getIdsFromViews(actual);
    ...
    public void testFindSliceByAgeGreaterThanEqual() {
        ...
        // When
        Slice<DocumentAccessor> actual = DocumentAccessors.of(documentRepository.findSliceByAgeGreaterThanEqual(4L, new PageRequest(1, 1, "age", "id")));
        List<Long> actualIds = getIdsFromViews(actual);
    ...
    public void testEntityViewAttributeSorting() {
        ...
        List<DocumentView> list = documentRepository.findAll(Sort.asc(sortProperty));

        assertEquals(doc3, list.get(0).getName());

        list = documentRepository.findAll(Sort.desc(sortProperty));
