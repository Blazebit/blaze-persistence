 ...
 *
 * NOTE: This was adapted from the original implementation to support the reactive model.
 * 
 * @since 1.6
...
public class PageableHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Mono<Object> resolveArgument(MethodParameter methodParameter, BindingContext bindingContext, ServerWebExchange serverWebExchange) {
        ...
        String pageString = serverWebExchange.getRequest().getQueryParams().getFirst(getParameterNameToUse(pageParameterName, methodParameter));
        String pageSizeString = serverWebExchange.getRequest().getQueryParams().getFirst(getParameterNameToUse(sizeParameterName, methodParameter));
        ...
        Mono<Sort> sort = (Mono<Sort>) (Mono<?>) sortResolver.resolveArgument(methodParameter, bindingContext, serverWebExchange);
        return sort.map(s -> Mono.just(PageRequest.of(p, size,
                s.isSorted() ? s : defaultOrFallback.map(Pageable::getSort).orElseGet(Sort::unsorted))));
    }
