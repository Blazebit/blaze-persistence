...
public class CustomTableBasedUpdateHandlerImpl implements MultiTableBulkIdStrategy.UpdateHandler {
    ...
    public int execute(SharedSessionContractImplementor s, QueryParameters queryParameters) {
        ...
        SessionImplementor sessionProxy = (SessionImplementor) Proxy.newProxyInstance(session.getClass().getClassLoader(), new Class[]{SessionImplementor.class, EventSource.class}, new Hibernate52SessionInvocationHandler((SessionImplementor) session, jdbcCoordinatorProxy));
