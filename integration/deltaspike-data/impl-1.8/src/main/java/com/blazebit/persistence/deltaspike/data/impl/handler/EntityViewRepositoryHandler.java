...
public class EntityViewRepositoryHandler<E, V, PK extends Serializable>
    ...
    public V findBy(PK pk) {
        ...
        if (context.getRepositoryMethodMetadata().getEntityViewClass() == null) {
            result = (List<V>) cb.getResultList();
        } else {
            result = context.getEntityViewManager().applySetting(
                    createSetting(),
                    cb
            ).getResultList();
        }
    ...
    public List<V> findAll(int start, int max) {
        if (context.getRepositoryMethodMetadata().getEntityViewClass() == null) {
            return (List<V>) createCriteriaBuilder()
                    .page(start, max)
    ...
    private EntityViewSetting<V, CriteriaBuilder<V>> createSetting() {
        return EntityViewSetting.create((Class<V>) context.getRepositoryMethodMetadata().getEntityViewClass());
    }
