...
//
    @Override
    public ExecutionContext wrapExecutionContext(Query exampleQuery, DbmsDialect dbmsDialect, int[] returningSqlTypes, HibernateReturningResult<?> returningResult) {
        QuerySqmImpl<?> hqlQuery = exampleQuery.unwrap(QuerySqmImpl.class);
        SessionImplementor session = (SessionImplementor) hqlQuery.getSession();
        JdbcCoordinator jdbcCoordinator = session.getJdbcCoordinator();

        Object jdbcCoordinatorProxy = Proxy.newProxyInstance(jdbcCoordinator.getClass().getClassLoader(), new Class[]{ JdbcCoordinator.class }, new JdbcCoordinatorInvocationHandler(jdbcCoordinator, session.getFactory(), dbmsDialect, returningSqlTypes, returningResult));
        ...
        Object exceutionContextProxy = Proxy.newProxyInstance(session.getClass().getClassLoader(), new Class[]{ ExecutionContext.class }, new Hibernate60ExecutionContextInvocationHandler(hqlQuery, (SessionImplementor) sessionProxy));
