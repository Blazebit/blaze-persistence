...
. alias if an explicit alias has been defined via link:{core_doc}/persistence/FromBuilder.html#joinDefault(java.lang.String,%20java.lang.String,%20com.blazebit.persistence.JoinType)[`joinDefault()`] means
...
If the property is the identifier and the `JpaProvider` supports link:{core_doc}/persistence/spi/JpaProvider.html#supportsSingleValuedAssociationIdExpressions()[optimized id access],
...
An explicit fetch can be invoked on link:{core_doc}/persistence/FullQueryBuilder.html[`FullQueryBuilder`] instances which is the top type for
link:{core_doc}/persistence/CriteriaBuilder.html[`CriteriaBuilder`] and link:{core_doc}/persistence/PaginatedCriteriaBuilder.html[`PaginatedCriteriaBuilder`].
...
link:{core_doc}/persistence/FromBuilder.html#joinDefault(java.lang.String,%20java.lang.String,%20com.blazebit.persistence.JoinType)[`joinDefault`] method and variants.
...
Analogous to the `FETCH` keyword in JPQL, you can specify for every join node of a link:{core_doc}/persistence/FullQueryBuilder.html[`FullQueryBuilder`] if it should be fetched.
Every link:{core_doc}/persistence/FromBuilder.html#join(java.lang.String,%20java.lang.String,%20com.blazebit.persistence.JoinType)[`join`] method variant comes with a partner method,
that does fetching for the joined path. In addition to that, there is also a simple link:{core_doc}/persistence/FullQueryBuilder.html#fetch(java.lang.String...)[`fetch`] method which can be provided with absolute paths to relations.
...
A correlated join in {projectname} can be done when link:{core_doc}/persistence/SubqueryInitiator.html#from(java.lang.String)[initiating a subquery]
or be added as link:{core_doc}/persistence/BaseSubqueryBuilder.html#from(java.lang.String)[cross join] to an existing subquery builder.
...
Entity joins normally link:{core_doc}/persistence/FromBuilder.html#joinOn(java.lang.String,%20java.lang.Class,%20java.lang.String,%20com.blazebit.persistence.JoinType)[require a base alias]
but link:{core_doc}/persistence/FromBuilder.html#joinOn(java.lang.Class,%20java.lang.String,%20com.blazebit.persistence.JoinType)[default to the query root] when only a single query root is available.
...
In such cases use one of the link:{core_doc}/persistence/FromBuilder.html#fromValues(java.lang.Class,%20java.lang.String,%20java.util.Collection)[overloads] that use the collection size as number of elements.
...
The resulting JPQL looks similar to how the ANSI SQL representation would look like
...
// TODO: for more infor on TREAT refer to custom jpql functions chapter
...
----
SELECT myValue
FROM MyEmbeddable(VALUES (?,?)) myValue
----

Since the type `MyEmbeddable` has 2 properties, the query also contains two parameter markers per object.
...
link:{core_doc}/persistence/FromBuilder.html#fromIdentifiableValues(java.lang.Class,%20java.lang.String,%20java.util.Collection)[`fromIdentifiableValues()`].

The main difference to the link:{core_doc}/persistence/FromBuilder.html#fromValues(java.lang.Class,%20java.lang.String,%20java.util.Collection)[managed values variant]
...
Instead of having a parameter for every property of a `Cat`, the `VALUES` clause only includes the identifier part.
...
When it is necessary to do that, it is strongly advised to make use of link:{core_doc}/persistence/FromBuilder.html#fromOld(java.lang.Class)[`fromOld()`]
or link:{core_doc}/persistence/FromBuilder.html#fromNew(java.lang.Class)[`fromNew()`] to use the state before or after side-effects happen.
