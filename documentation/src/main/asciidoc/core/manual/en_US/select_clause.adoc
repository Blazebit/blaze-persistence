...
When selecting multiple expressions, you can decide between the query return types `Object[]` and `jakarta.persistence.Tuple`.

WARNING: Some persistence providers might return the object directly when having just a single select item, even if you pass `Object[]`. This will not happen when using `jakarta.persistence.Tuple` as query return type.
