...
[[environments-wildfly]]
==== Wildfly

Blaze Persistence can be integrated with WildFly 36 in standalone mode using JDK 21, Jakarta EE 10 JPA (JPA 3.1), and Hibernate 6.6.7 as the JPA provider.

Define a WildFly module for Blaze Persistence

Create a module directory:

`$WILDFLY/modules/system/layers/base/com/blazebit/main/`

Place the following JARs inside the folder:

- blaze-persistence-core-api-jakarta-{version}.jar
- blaze-persistence-core-impl-jakarta-{version}.jar
- blaze-persistence-core-parser-jakarta-{version}.jar
- blaze-persistence-integration-hibernate-6.2-{version}.jar
- blaze-persistence-integration-hibernate6-base-{version}.jar
- blaze-persistence-integration-jpa-base-jakarta-{version}.jar
- blaze-common-utils-0.1.21.jar
...
<module name="com.blazebit" xmlns="urn:jboss:module:1.9">
    <resources>
        <resource-root path="blaze-persistence-core-api-jakarta-{version}.jar"/>
        <resource-root path="blaze-persistence-core-impl-jakarta-{version}.jar"/>
        <resource-root path="blaze-persistence-core-parser-jakarta-{version}.jar"/>
        <resource-root path="blaze-persistence-integration-hibernate-6.2-{version}.jar"/>
        <resource-root path="blaze-persistence-integration-hibernate6-base-{version}.jar"/>
        <resource-root path="blaze-persistence-integration-jpa-base-jakarta-{version}.jar"/>
...
Update `META-INF/jboss-deployment-structure.xml` of the client module

In the module that uses Blaze Persistence (client module), in the above-mentioned file, include:
...
⚠️ `services="import"` is required for proper service injection.

Add Blaze Persistence dependencies to your client module POM
...
import com.bootstrap.EntityManagerProvider;
...
@Startup
public class CriteriaBuilderFactoryProducer {
    private static volatile CriteriaBuilderFactory criteriaBuilderFactory;

    @PostConstruct
    public void init() {
        CriteriaBuilderConfiguration config = Criteria.getDefault();

        EntityManagerFactory emf = EntityManagerProvider.getEntityManagerFactory("datasource-name");

        criteriaBuilderFactory = config.createCriteriaBuilderFactory(emf);
    }

    public static CriteriaBuilderFactory get() {
        if (criteriaBuilderFactory == null) {
            throw new IllegalStateException("CriteriaBuilderFactory is not initialized yet!");
        }
        return criteriaBuilderFactory;
    }
}
----
then use in any method of any class with:
[source,java]
----
CriteriaBuilderFactory cbf = CriteriaBuilderFactoryProducer.get();
----
An instance of JPA's `EntityManager` is to be provided as well before doing the query.
...
Once the link:{core_jdoc}/persistence/CriteriaBuilderFactory.html#create(jakarta.persistence.EntityManager,%20java.lang.Class)[`create()`] method is called the expression
...
link:{core_jdoc}/persistence/CriteriaBuilderFactory.html#create(jakarta.persistence.EntityManager,%20java.lang.Class)[`create()`] method and denotes the result type of the query.
The default behavior of link:{core_jdoc}/persistence/CriteriaBuilderFactory.html#create(jakarta.persistence.EntityManager,%20java.lang.Class)[`create()`] is that the result type
...
writing, both the link:{core_jdoc}/persistence/CriteriaBuilderFactory.html#create(jakarta.persistence.EntityManager,%20java.lang.Class)[`create()`] and
