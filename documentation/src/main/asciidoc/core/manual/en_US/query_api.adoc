...
link:{core_jdoc}/persistence/CriteriaBuilderFactory.html#create(jakarta.persistence.EntityManager,%20java.lang.Class)[`CriteriaBuilder`] instances for `SELECT` statements.

The top level query builders allow access to the query string and also to construct JPA `jakarta.persistence.Query` or `jakarta.persistence.TypedQuery` instances through the interfaces
