...
methods like `jakarta.persistence.Query.setMaxResults()` or `jakarta.persistence.Query.setFirstResult()` to steer the result set
...
. The *ID query* is used to fetch the entity ids that are contained in the target page. In this step, the previously
 mentioned `jakarta.persistence.Query.setMaxResults()` and `jakarta.persistence.Query.setFirstResult()` are used to select the
