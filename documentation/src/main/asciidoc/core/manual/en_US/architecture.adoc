...
The query specification is wrapped in an implementation of the JPA query interfaces `jakarta.persistence.Query` or `jakarta.persistence.TypedQuery` and a query plan is only created on demand just before executing.
