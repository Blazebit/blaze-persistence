...
These keysets can also be used for paging/scrolling although when already having access to a link:{core_doc}/persistence/KeysetPage.html[`KeysetPage`] it might be better to use the
link:{core_doc}/persistence/PaginatedQueryBuilder.html[`PaginatedQueryBuilder`] API instead.
