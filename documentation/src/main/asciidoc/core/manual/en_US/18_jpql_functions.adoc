...
WARNING: `WEEK` and `ISO_WEEK` return the ISO based week number, which may be in the previous year, whereas `WEEK_IN_YEAR` returns the week number counting from the first day of the year. To determine the year of that the ISO based week started on, use the `YEAR_OF_WEEK`, or use the `YEAR_WEEK` which returns the week and its year as a string in `YYYY-MM` format.
