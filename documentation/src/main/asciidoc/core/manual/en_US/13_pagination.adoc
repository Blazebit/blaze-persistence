...
The inlining can be forced or disabled by passing `true` or `false` to `PaginatedCriteriaBuilder.withInlineIdQuery(boolean)`.

NOTE: In a future version it might be possible to also inline the count query. For more information on that see https://github.com/Blazebit/blaze-persistence/issues/249[#249].
