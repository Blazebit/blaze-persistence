...
Some of the basics like link:documentation/core/manual/en_US/index.html#implicit-joins[implicit joins]
and the basic link:documentation/core/manual/en_US/index.html#expressions[expression structure] should be known to understand all of the following mapping examples.
...
Although most example only use path expressions for the mapping, it is actually allowed to use any link:documentation/core/manual/en_US/index.html#expressions[scalar expression] that JPQL or {projectname} allows.
...
In order to understand the following generated query, you should know what an link:documentation/core/manual/en_US/index.html#implicit-joins[implicit join] does and how entity views make use of such implicit joins.
...
Since the `father` relation is optional or nullable, a (default) left join is created due to the rules of link:documentation/core/manual/en_US/index.html#model-awareness[model awareness] in implicit joins.
...
Subquery mappings allow to map scalar subqueries into entity views and are declared by annotating the desired attribute with `@MappingSubquery` and specifying a link:{entity_view_doc}/persistence/view/SubqueryProvider.html[`SubqueryProvider`].
...
This entity view already comes into contact with the link:documentation/core/manual/en_US/index.html[core] API for creating subqueries.
...
For more information on this check out the documentation of the link:documentation/core/manual/en_US/index.html#outer-function[`OUTER` function]
