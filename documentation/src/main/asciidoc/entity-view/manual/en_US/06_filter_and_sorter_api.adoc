...
{projectname} provides a number of built-in filter providers in the
...
[NOTE]
{projectname} currently does not support filters for collections with "has at least one" semantics. This is
...
{projectname} provides default sorters via the static methods in `com.blazebit.persistence.view.Sorters`. They
