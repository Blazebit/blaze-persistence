...
*save*::: Flushing/Updating happens when invoking `EntityViewManager.save()`/`EntityViewManager.saveWith()` or `EntityViewManager.saveFull()`/`EntityViewManager.saveFullWith()` as well as implicitly for `CascadeType.UPDATE` enabled attributes.
...
Global listeners are registered via one of the `EntityViewConfiguration.registerListener` methods or discovered in a CDI or Spring environment
