...
An owned *ToOne relationship is a _link_ to an existing object which shouldn't normally be altered as part of the object owning the *ToOne relationship.
...
As there might be models out there, that do not fit this requirement, it is possible to disable this strict check via link:{entity_view_doc}#updater_disallow_owned_updatable_subview[configuration property].
