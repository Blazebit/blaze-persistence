...
and the support for chunk-processing results through the new `Stream<T> getResultStream()` method that was also introduced with JPA 2.2 on `jakarta.persistence.Query`.
