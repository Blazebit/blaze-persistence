...
public class TransientEntityAssociationParameterTransformerFactory implements AssociationParameterTransformerFactory {
    ...
    @Override
    public ParameterManager.ParameterValueTranformer getToEntityTranformer(Class<?> associationType) {
        IdentifiableType<?> managedType = (IdentifiableType<?>) metamodel.getManagedType(associationType);
        Attribute<?, ?> idAttribute = JpaUtils.getIdAttribute(managedType);
        return AssociationFromIdParameterTransformer.getInstance(associationType, idAttribute);
    }
