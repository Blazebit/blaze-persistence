...
public class PostgreSQLMinuteDiffFunction extends MinuteDiffFunction {
    ...
    public PostgreSQLMinuteDiffFunction() {
        super("(select date_part('day', t2::timestamp - t1::timestamp) * " + (24 * 60) + " + date_part('hour', t2::timestamp - t1::timestamp) * 60 + date_part('minute', t2::timestamp - t1::timestamp) from (values (?1,?2)) as temp(t1,t2))");
    }
