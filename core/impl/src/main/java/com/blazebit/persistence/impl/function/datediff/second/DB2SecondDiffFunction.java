...
public class DB2SecondDiffFunction extends SecondDiffFunction {
    ...
    public DB2SecondDiffFunction() {
        ...
        super("(select (days(cast(t2 as timestamp)) - days(cast(t1 as timestamp))) * " + (24 * 60 * 60) + " + (midnight_seconds(cast(t2 as timestamp)) - midnight_seconds(cast(t1 as timestamp))) from lateral(values (?1,?2)) as temp(t1,t2))");
    }
