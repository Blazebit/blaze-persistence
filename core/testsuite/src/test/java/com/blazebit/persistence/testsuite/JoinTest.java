...
public class JoinTest extends AbstractCoreTest {
    ...
    public void testPaginatedJoinFetch(){
        ...
        if (jpaProvider.supportsSubqueryInFunction() && jpaProvider.supportsSubqueryAliasShadowing()) {
            assertEquals("SELECT d, (" + expectedCountQuery + ") FROM Document d LEFT JOIN FETCH d.contacts c WHERE d.id IN (" + expectedIdQuery + " LIMIT 10) ORDER BY d.id ASC", crit.withInlineIdQuery(true).withInlineCountQuery(true).getQueryString());
        }
    }
    ...
    public void testSizeNoExplicitJoinReusal() {
        ...
        // Then
        final String expected = "SELECT " + countDistinct("partners_1.id") + " FROM Document d LEFT JOIN d.partners partners_1 LEFT JOIN d.partners partner WHERE partner.name = 'Joe' GROUP BY d.id";
        assertEquals(expected, crit.getQueryString());
