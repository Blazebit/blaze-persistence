...
public class LimitTest extends AbstractCoreTest {
    ...
    public void testSubqueryLimit() {
        ...
        .end()
        .setMaxResults(1);
        String expected = "SELECT p FROM Person p WHERE p.id IN (" + function("LIMIT", "(SELECT pSub.id FROM Person pSub ORDER BY pSub.id ASC NULLS LAST)", "1") + ")";
    ...
    public void testSubqueryAndOuterQueryLimit() {
    	...
    	.orderByAsc("p.id")
        .setMaxResults(1);
        String expected = "SELECT p FROM Person p WHERE p.id IN (" + function("LIMIT", "(SELECT pSub.id FROM Person pSub ORDER BY pSub.id ASC NULLS LAST)", "2") + ") ORDER BY p.id ASC NULLS LAST";
