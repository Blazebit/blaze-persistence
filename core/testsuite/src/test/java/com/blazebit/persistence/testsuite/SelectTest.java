...
public class SelectTest extends AbstractCoreTest {
    ...
    public void testSelectSizeAsDistinctCount2() {
        ...
        // Then
        String expected = "SELECT CASE WHEN " + function("COUNT_TUPLE", "'DISTINCT'", "d.id", "KEY(contacts_1)") + " > 2 THEN " + function("COUNT_TUPLE", "'DISTINCT'", "d.id", "partners_1") + " ELSE " + function("COUNT_TUPLE", "'DISTINCT'", "d.id", "versions_1") + " END FROM Document d LEFT JOIN d.contacts contacts_1 LEFT JOIN d.partners partners_1 LEFT JOIN d.versions versions_1 GROUP BY d.id";
        assertEquals(expected, cb.getQueryString());
    ...
    public void testSelectAggregate() {
        ...
        String objectQuery = "SELECT " + function("COUNT_TUPLE", "d.id", "versions_1") + ", owner_1.name FROM Document d JOIN d.owner owner_1 LEFT JOIN d.versions versions_1 GROUP BY d.id, owner_1.name ORDER BY " + renderNullPrecedence("d.id", "DESC", "LAST");
    ...
    public void testSelectAggregatePaginated() {
        ...
        String objectQuery = "SELECT " + function("COUNT_TUPLE", "d.id", "versions_1") + ", owner_1.name FROM Document d JOIN d.owner owner_1 LEFT JOIN d.versions versions_1 WHERE d.id IN :ids GROUP BY d.id, owner_1.name ORDER BY " + renderNullPrecedence("d.id", "DESC", "LAST");
