...
//                + "(SELECT p.id FROM Person p WHERE p.name = n)").getResultList();
        ...
        em.createQuery(
            "SELECT d.name, UPPER(d.name) FROM Document d WHERE d.age = (SELECT AVG(d.age) FROM Document d2 WHERE d.id IN (SELECT p.id FROM Person p))")
            .getResultList();

        em.createQuery("SELECT d.id, d.name, contacts.name FROM Document d LEFT JOIN d.contacts contacts ORDER BY d.name")
            .getResultList();
