...
public class TablePerClassEmbeddableSub2 implements Sub2Embeddable<TablePerClassBase>, Serializable {
    ...
    // We can't have a constraint in this case because we don't know the exact table this will refer to
    @JoinTable(name = "tpces2_list", foreignKey = @jakarta.persistence.ForeignKey(ConstraintMode.NO_CONSTRAINT), inverseForeignKey = @jakarta.persistence.ForeignKey(ConstraintMode.NO_CONSTRAINT))
    ...
    @ManyToMany
    // We can't have a constraint in this case because we don't know the exact table this will refer to
    @JoinTable(name = "tpces2_map", foreignKey = @jakarta.persistence.ForeignKey(ConstraintMode.NO_CONSTRAINT), inverseForeignKey = @jakarta.persistence.ForeignKey(ConstraintMode.NO_CONSTRAINT))
