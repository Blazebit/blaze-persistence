 ...
 * @since 1.2.0
 */
public class CorrelatedBasicBatchSetTupleListTransformerFactory extends AbstractCorrelatedBasicSubqueryTupleTransformerFactory {

    public CorrelatedBasicBatchSetTupleListTransformerFactory(Class<?> criteriaBuilderResult, ManagedViewType<?> viewRoot, String correlationResult, CorrelationProviderFactory correlationProviderFactory, String attributePath, int tupleIndex, int batchSize, Class<?> correlationBasisEntity) {
    ...
    public TupleListTransformer create(FullQueryBuilder<?, ?> queryBuilder, Map<String, Object> optionalParameters, EntityViewConfiguration entityViewConfiguration) {
        ...
        return new CorrelatedSetTupleListTransformer(entry.getKey(), entry.getValue(), paramName, tupleIndex, batchSize, correlationBasisEntity);
    }
