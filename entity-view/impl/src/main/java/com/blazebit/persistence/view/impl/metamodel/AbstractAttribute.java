...
public abstract class AbstractAttribute<X, Y> implements Attribute<X, Y> {
    ...
    private static enum ExpressionLocationKind {
        ...
        ExpressionLocationKind(String location) {
    ...
    private static void validateTypesCompatible(ManagedType<?> managedType, String expression, Class<?> targetType, Class<?> targetElementType, boolean subtypesAllowed, MetamodelBuildingContext context, ExpressionLocationKind expressionKind, String location) {
        ...
        if (expression.isEmpty()) {
            ...
            sb.append(']');
            context.addError("The resolved possible types " + sb.toString() + " are not assignable to the given expression type '" + expressionType.getName() + "' of the " + expressionKind + " declared by the " + location + "!");
            return;
        }
        ...
        try {
            ...
            context.addError("Syntax error in " + expressionKind + " '" + expression + "' of the " + location + ": " + ex.getMessage());
        } catch (IllegalArgumentException ex) {
            context.addError("An error occurred while trying to resolve the " + expressionKind + " of the " + location + ": " + ex.getMessage());
        }
    ...
    public void checkAttribute(ManagedType<?> managedType, Map<Class<?>, ManagedViewTypeImpl<?>> managedViews, MetamodelBuildingContext context) {
        ...
        if (isCorrelated()) {
            ...
            validateTypesCompatible(managedType, correlationBasis, Object.class, null, true, context, ExpressionLocationKind.CORRELATION_BASIS, getLocation());

            if (correlated != null) {
                // Validate that resolving "correlationResult" on "correlated" is compatible with "expressionType" and "elementType"
                validateTypesCompatible(context.getEntityMetamodel().managedType(correlated), correlationResult, expressionType, elementType, true, context, ExpressionLocationKind.CORRELATION_RESULT, getLocation());
                ...
                try {
                    ...
                    context.addError("Syntax error in " + ExpressionLocationKind.CORRELATION_EXPRESSION + " '" + correlationExpression + "' of the " + getLocation() + ": " + ex.getMessage());
                } catch (IllegalArgumentException ex) {
                    context.addError("An error occurred while trying to resolve the " + ExpressionLocationKind.CORRELATION_EXPRESSION + " of the " + getLocation() + ": " + ex.getMessage());
                }
            }
            ...
            validateTypesCompatible(managedType, mapping, expressionType, elementType, subtypesAllowed, context, ExpressionLocationKind.MAPPING, getLocation());
