 ...
 *
 * Christian Beikov: Adapted to track the local index a stack value belongs to.
 *
...
public class Executor implements Opcode {
    ...
    public void execute(MethodInfo method, int pos, CodeIterator iter, Frame frame, Subroutine subroutine) throws BadBytecode {
        ...
        switch (opcode) {
            ...
            case DUP: {
                ...
                frame.push(frame.peek(), frame.peekOrigin());
                break;
            }
            case DUP_X1:
            case DUP_X2: {
                ...
                if (type == Type.TOP)
                    throw new BadBytecode("DUP can not be used with a category 2 value, pos = " + pos);
                Integer originIndex = frame.peekOrigin();
                int end = frame.getTopIndex();
                int insert = end - (opcode - DUP_X1) - 1;
                frame.push(type, originIndex);

                while (end > insert) {
                    frame.setStack(end, frame.getStack(end - 1), frame.getStackOrigin(end - 1));
                    end--;
                }
                frame.setStack(insert, type, originIndex);
                break;
            }
            case DUP2:
                frame.push(frame.getStack(frame.getTopIndex() - 1), frame.getStackOrigin(frame.getTopIndex() - 1));
                frame.push(frame.getStack(frame.getTopIndex() - 1), frame.getStackOrigin(frame.getTopIndex() - 1));
                break;
            case DUP2_X1:
            case DUP2_X2: {
                ...
                Integer origin1 = frame.getStackOrigin(frame.getTopIndex() - 1);
                Integer origin2 = frame.peekOrigin();
                frame.push(type1, origin1);
                frame.push(type2, origin2);
                while (end > insert) {
                    frame.setStack(end, frame.getStack(end - 2), frame.getStackOrigin(end - 2));
                    end--;
                }
                frame.setStack(insert, type2, origin2);
                frame.setStack(insert - 1, type1, origin1);
                break;
            }
            case SWAP: {
                Integer origin1 = frame.peekOrigin();
            ...
            // Conversion
            case I2L: {
                Integer origin = frame.peekOrigin();
                verifyAssignable(Type.INTEGER, simplePop(frame));
                simplePush(Type.LONG, frame, origin);
                break;
            }
            case I2F: {
                Integer origin = frame.peekOrigin();
                verifyAssignable(Type.INTEGER, simplePop(frame));
                simplePush(Type.FLOAT, frame, origin);
                break;
            }
            case I2D: {
                Integer origin = frame.peekOrigin();
                verifyAssignable(Type.INTEGER, simplePop(frame));
                simplePush(Type.DOUBLE, frame, origin);
                break;
            }
            case L2I: {
                Integer origin = frame.peekOrigin();
                verifyAssignable(Type.LONG, simplePop(frame));
                simplePush(Type.INTEGER, frame, origin);
                break;
            }
            case L2F: {
                Integer origin = frame.peekOrigin();
            ...
            case ATHROW:
                ...
                break;
            case CHECKCAST:
                Integer castOrigin = frame.peekOrigin();
                verifyAssignable(Type.OBJECT, simplePop(frame));
                frame.push(typeFromDesc(constPool.getClassInfoByDescriptor(iter.u16bitAt(pos + 1))), castOrigin);
                break;
            case INSTANCEOF:
                verifyAssignable(Type.OBJECT, simplePop(frame));
                frame.push(Type.INTEGER);
    ...
    private void simplePush(Type type, Frame frame, Integer origin) {
        frame.push(type, origin);
        if (type.getSize() == 2)
            frame.push(Type.TOP);
    }
