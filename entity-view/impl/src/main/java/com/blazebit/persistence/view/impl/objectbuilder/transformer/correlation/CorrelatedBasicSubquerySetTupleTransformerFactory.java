 ...
 * @since 1.2.0
 */
public class CorrelatedBasicSubquerySetTupleTransformerFactory extends AbstractCorrelatedBasicSubqueryTupleTransformerFactory {

    public CorrelatedBasicSubquerySetTupleTransformerFactory(Class<?> criteriaBuilderResult, ManagedViewType<?> viewRoot, String correlationResult, CorrelationProviderFactory correlationProviderFactory, int tupleIndex, Class<?> correlationBasisEntity) {
    ...
    public TupleTransformer create(FullQueryBuilder<?, ?> queryBuilder, Map<String, Object> optionalParameters) {
        ...
        return new CorrelatedSetTupleTransformer(entry.getKey(), entry.getValue(), paramName, tupleIndex, correlationBasisEntity);
    }
