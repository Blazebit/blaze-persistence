 ...
 * @since 1.2.0
 */
public class CorrelatedBasicSubqueryOrderedSetTupleTransformerFactory extends AbstractCorrelatedBasicSubqueryTupleTransformerFactory {

    public CorrelatedBasicSubqueryOrderedSetTupleTransformerFactory(Class<?> criteriaBuilderResult, ManagedViewType<?> viewRoot, String correlationResult, CorrelationProviderFactory correlationProviderFactory, int tupleIndex, Class<?> correlationBasisEntity) {
    ...
    public TupleTransformer create(FullQueryBuilder<?, ?> queryBuilder, Map<String, Object> optionalParameters) {
        ...
        return new CorrelatedOrderedSetTupleTransformer(entry.getKey(), entry.getValue(), paramName, tupleIndex, correlationBasisEntity);
    }
