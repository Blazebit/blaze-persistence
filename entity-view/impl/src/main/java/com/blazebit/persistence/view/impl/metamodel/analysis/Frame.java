...
public class Frame {
    ...
    public Frame(int locals, int stack) {
        ...
        this.stackOrigin = new Integer[stack];
    }
    ...
    public Integer getStackOrigin(int index) {
        return stackOrigin[index];
    }
     ...
     * @param originIndex The parameter origin index
     */
    public void setStack(int index, Type type, Integer originIndex) {
        stack[index] = type;
        stackOrigin[index] = originIndex;
    }
    ...
    public Integer peekOrigin() {
        return stackOrigin[top - 1];
    }
    ...
    public void push(Type type) {
        stackOrigin[top] = null;
        stack[top++] = type;
    }
    public void push(Type type, Integer originIndex) {
        stackOrigin[top] = originIndex;
        stack[top++] = type;
    }
    ...
    public Frame copy() {
        ...
        System.arraycopy(stackOrigin, 0, frame.stackOrigin, 0, stackOrigin.length);
        frame.top = top;
        return frame;
    }
    ...
    public boolean mergeStack(Frame frame) {
        ...
        for (int i = 0; i < top; i++) {
            if (stack[i] != null) {
                ...
                stack[i] = merged;
                if (stackOrigin[i] == null) {
                    stackOrigin[i] = frame.stackOrigin[i];
                } else if (!stackOrigin[i].equals(frame.stackOrigin[i])) {
                    stackOrigin[i] = null;
                }
                // always replace the instance in case a multi-interface type changes to a normal Type
                if ((! merged.equals(prev)) || TypeUtils.popChanged(merged)) {
                    changed = true;
                }
            }
        }
