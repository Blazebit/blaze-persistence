 ...
 * @since 1.2.0
 */
public class CorrelatedSubviewSubquerySetTupleTransformerFactory<T> extends AbstractCorrelatedSubviewSubqueryTupleTransformerFactory<T> {

    public CorrelatedSubviewSubquerySetTupleTransformerFactory(ManagedViewType<T> managedViewType, ManagedViewType<?> viewRootType, CorrelationProviderFactory correlationProviderFactory, int tupleIndex, EntityViewManagerImpl evm, ExpressionFactory ef, String viewName) {
    ...
    public TupleTransformer create(FullQueryBuilder<?, ?> queryBuilder, Map<String, Object> optionalParameters) {
        ...
        return new CorrelatedSetTupleTransformer(entry.getKey(), entry.getValue(), paramName, tupleIndex);
    }
