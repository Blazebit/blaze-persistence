...
public class CustomLineNumberAttribute extends AttributeInfo {
    /**
     * The name of this attribute <code>"LineNumberTable"</code>.
     */
    public static final String tag = "LineNumberTable";

    public CustomLineNumberAttribute(ConstPool cp, byte[] i) {
        super(cp, tag, i);
    }
    ...
    public int startPc(int i) {
        return ByteArray.readU16bit(get(), i * 4 + 2);
    }
    ...
    public int lineNumber(int i) {
        return ByteArray.readU16bit(get(), i * 4 + 4);
    }
    ...
    public AttributeInfo copy(ConstPool newCp, Map classnames) {
        ...
        CustomLineNumberAttribute attr = new CustomLineNumberAttribute(newCp, dest);
        return attr;
    }
    ...
    public void shiftPc(int where, int gapLength, boolean exclusive) {
        ...
        for (int i = 0; i < n; ++i) {
            int pos = i * 4 + 2;
            int pc = ByteArray.readU16bit(get(), pos);
            if (pc > where || (exclusive && pc == where))
                ByteArray.write16bit(pc + gapLength, get(), pos);
        }
    }
}