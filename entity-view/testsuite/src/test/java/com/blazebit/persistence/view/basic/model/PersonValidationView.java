...
public interface PersonValidationView extends IdHolderView<Long> {
    ...
    public String getOwnerDocumentNames();
    ...
    @Mapping("COALESCE(localized[:language].name, localized[defaultLanguage].name)")
    public String getName();
    
    @Mapping("CASE WHEN age = 9 THEN 0 ELSE CASE WHEN partnerDocument.creationDate < FUNCTION('current_timestamp') THEN 3 "

    + " ELSE CASE WHEN partnerDocument.creationDate < FUNCTION('timestamp_add_day', FUNCTION('current_timestamp'), person.age) THEN 3"

    + " ELSE CASE WHEN partnerDocument.creationDate < FUNCTION('timestamp_add_day', FUNCTION('current_timestamp'), person.id) THEN 2 ELSE 1 END END END END")
